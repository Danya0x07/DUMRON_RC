...
typedef enum {NRF_OPERATION_READ, NRF_OPERATION_WRITE} NrfOperation;

#define nrf_csn_1() GPIO_WriteHigh(NRF_GPIO, NRF_PIN_CSN)
#define nrf_csn_0() GPIO_WriteLow(NRF_GPIO, NRF_PIN_CSN)
#define nrf_ce_1()  GPIO_WriteHigh(NRF_GPIO, NRF_PIN_CE)
#define nrf_ce_0()  GPIO_WriteLow(NRF_GPIO, NRF_PIN_Ce)

uint8_t nrf_communicate_via_spi(uint8_t byte);
uint8_t nrf_cmd(NrfCmd cmd);
uint8_t nrf_overwrite_byte(NrfRegAddress reg_addr, uint8_t bit_flags);
uint8_t nrf_read_byte(NrfRegAddress reg_addr);
uint8_t nrf_rwx(uint8_t composite_cmd, uint8_t* buf, uint8_t size, NrfOperation operation);
uint8_t nrf_rmw(NrfRegAddress reg_addr, uint8_t bit_value, BitStatus bit_status);

#define nrf_get_status()  nrf_cmd(NOP)
#define nrf_is_irq()  ((NRF_PIN_CEIRQ & NRF_IRQ) == 0)
#define nrf_clear_interrupts()  nrf_rmw(STATUS, 0, RESET)
#define nrf_read_to_buffer(reg_addr, buf_addr, size) \
    nrf_rwx(R_REGISTER | ((reg_addr) & 0x1F), (buf_addr), (size), NRF_OPERATION_READ)
#define nrf_write_from_buffer(reg_addr, buf_addr, size) \
    nrf_rwx(W_REGISTER | ((reg_addr) & 0x1F), (buf_addr), (size), NRF_OPERATION_WRITE)
...
uint8_t nrf_cmd(NrfCmd cmd)
{
    uint8_t status;
    nrf_csn_0();
    status = nrf_communicate_via_spi(cmd);
    nrf_csn_1();
    return status;
}
...
uint8_t nrf_overwrite_byte(NrfRegAddress reg_addr, uint8_t bit_flags)
{
    uint8_t status;
    nrf_csn_0();
    status = nrf_communicate_via_spi(W_REGISTER | (reg_addr & 0x1F));
    nrf_communicate_via_spi(bit_flags);
    nrf_csn_1();
    return status;
}
...
uint8_t nrf_read_byte(NrfRegAddress reg_addr)
{
    uint8_t reg_value;
    nrf_csn_0();
    nrf_communicate_via_spi(R_REGISTER | (reg_addr & 0x1F));
    reg_value = nrf_communicate_via_spi(NOP);
    nrf_csn_1();
    return reg_value;
}
 ...
 * @retval Значение регистра STATUS радиомодуля.
 */
uint8_t nrf_rmw(NrfRegAddress reg_addr, uint8_t bit_flags, BitStatus bit_status)
{
    uint8_t reg_value;
    uint8_t status = nrf_read_to_buffer(reg_addr, &reg_value, 1);
    if (bit_status == SET)
        reg_value |= bit_flags;
    else 
        reg_value &= ~bit_flags;
 ...
 * @retval Значение регистра STATUS радиомодуля.
 */
uint8_t nrf_rwx(uint8_t composite_cmd, uint8_t* buff, uint8_t size, NrfOperation operation)
{
    uint8_t status;
    nrf_csn_0();
    status = nrf_communicate_via_spi(composite_cmd);
    while (size--) {
        if (operation == NRF_OPERATION_READ)
            *buff++ = nrf_communicate_via_spi(NOP);
        else if (operation == NRF_OPERATION_WRITE)
            nrf_communicate_via_spi(*buff++);
    }
    nrf_csn_1();
    return status;
}
